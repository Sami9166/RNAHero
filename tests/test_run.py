...
class RunTests(unittest.TestCase):
    ...
    def test_one_command_prepares_four_cohorts_and_runs_analysis(self, search, metadata, download, crosswalk, assessment, selection, pipeline, critic) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertTrue((root / "analysis" / "analysis_config.json").is_file())
            self.assertTrue((root / "raw" / "GSE4" / "matrix.csv").is_file())
            self.assertTrue((root / "analysis" / "GSE4" / "crosswalk.json").is_file())
            ...
            self.assertTrue((root / "analysis" / "internal_validation.json").is_file())
            self.assertTrue((root / "analysis" / "internal_validation.csv").is_file())
            self.assertTrue((root / "analysis" / "external_validation.json").is_file())
            self.assertTrue((root / "analysis" / "external_validation.csv").is_file())
            self.assertEqual(json.loads((root / "analysis" / "critic_report.json").read_text(encoding="utf-8"))["verdict"], "caution")
            self.assertEqual(result["critic"]["engine"], "google-adk")
            self.assertEqual(json.loads((root / "search_results.json").read_text(encoding="utf-8")), [{"gse_id": f"GSE{i}", "title": f"study {i}"} for i in range(1, 5)])
            self.assertEqual(pipeline.call_count, 1)
            self.assertEqual(pipeline.call_args.args[0], root / "analysis" / "analysis_config.json")
